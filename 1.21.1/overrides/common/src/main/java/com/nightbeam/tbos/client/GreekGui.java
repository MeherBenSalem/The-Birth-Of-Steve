...
public final class GreekGui {
    ...
    private static final ResourceLocation PANELS = ResourceLocation.fromNamespaceAndPath("tbos", "textures/gui/greek/panels.png");
    private static final ResourceLocation ORNAMENTS = ResourceLocation.fromNamespaceAndPath("tbos", "textures/gui/greek/ornaments.png");
    ...
    public static void panel(GuiGraphics g,int tile,int x,int y,int w,int h) {
        if(w<=0||h<=0)return;
    ...
    public static void material(GuiGraphics g,int tile,int x,int y,int w,int h) {
        region(g,PANELS,x,y,w,h,tile%4*PANEL_SIZE/4+112,tile/4*PANEL_SIZE/4+112,88,88,PANEL_SIZE,PANEL_SIZE);
    }

    public static void progress(GuiGraphics g,int x,int y,int w,int h,float fraction,boolean complete) {
        material(g,TRACK,x,y,w,h);
        int filled=Math.round(w*Math.max(0,Math.min(1,fraction)));
        material(g,complete?GOLD:TEAL,x,y,filled,h);
    }

    public static void ornament(GuiGraphics g,int id,int x,int y,int w,int h) {
    ...
    private static void region(GuiGraphics g,ResourceLocation texture,int x,int y,int w,int h,
