# Overrides for Minecraft 26.1.2
...
Use this for the handful of things a Minecraft version genuinely has to do
differently — a mixin whose target signature moved, an access transformer entry,
a changed vanilla API call — not as a place to fork the mod.

Check what is in effect:

```bash
gradlew.bat -p 26.1.2 sourceOverrides
```

26.1.2 is the mainline target, so this directory is expected to stay empty.
