...
public final class MemoryLanternBlockEntity extends BlockEntity {
    ...
    public void load(CompoundTag input) {
        ...
        playbackTicks = Mth.clamp(input.getInt("playback_ticks"), 0, PLAYBACK_DURATION_TICKS - 1);
    }
    ...
    protected void saveAdditional(CompoundTag output) {
        ...
        if (scene != null) {
            output.put("scene", MemoryScene.CODEC.encodeStart(NbtOps.INSTANCE, scene).result().orElseThrow());
        }
