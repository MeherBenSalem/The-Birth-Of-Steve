...
public final class ArchivistQuestScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        // 1.20.1's Screen.render() paints nothing behind the screen: drawing the
        // backdrop is the caller's job here, unlike 1.21+ where Screen.render()
        // applies it along with the world blur. Without this call the journal
        // floats over the live world. It has to come before the journal is
        // composed, or it would darken the parchment, text, icons and progress
        // bars along with the world behind them.
        ...
        graphics.blitNineSliced(WELL, wellX, wellY, wellWidth, wellHeight, 3, 16, 16, 0, 0);
    ...
    private void progressBar(GuiGraphics graphics, int x, int y, int width, float fraction) {
        int height = 8;
        graphics.blitNineSliced(BAR_TRACK, x, y, width, height, 2, 8, 8, 0, 0);
        int filled = Math.round(width * Mth.clamp(fraction, 0.0F, 1.0F));
        if (filled >= height) {
            graphics.blitNineSliced(BAR_FILL, x, y, filled, height, 2, 8, 8, 0, 0);
        }
    }
