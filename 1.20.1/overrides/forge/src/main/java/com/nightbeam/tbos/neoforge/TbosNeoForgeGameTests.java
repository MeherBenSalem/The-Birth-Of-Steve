...
/**
 * Forge 1.20.1 GameTest bridge.
 *
 * <p>Forge scans {@link GameTestHolder} classes during its GameTest server
 ...
 * because 1.20.1 has no TestFunction data registry to mirror the 26.x targets.
 *
 * <p>The template is spelled without a namespace. Forge composes the structure
 * name as {@code getTemplateNamespace(method) + ":" + template}, taking the
 * namespace from {@link GameTestHolder}, so {@code "empty"} already resolves to
 * {@code tbos:empty}; spelling it out here would yield {@code tbos:tbos:empty}.
 * {@code @PrefixGameTestTemplate(false)} suppresses only the class-name prefix,
 * which would otherwise make it {@code tbos:tbosneoforgegametests.empty}.
 */
@GameTestHolder("tbos")
