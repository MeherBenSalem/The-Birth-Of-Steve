...
public final class LenswardEntity extends Monster {
    ...
    protected void registerGoals() {
        ...
        targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(
                this, Player.class, 10, true, false, (candidate, level) -> withinWard(candidate)));
    }

    @Override
    protected void defineSynchedData(SynchedEntityData.Builder entityData) {
        super.defineSynchedData(entityData);
        entityData.define(BEAM_PHASE, (byte) BeamPhase.IDLE.ordinal());
        entityData.define(BEAM_PHASE_TICKS, 0);
    }
    ...
    private static final class FocusedBeamGoal extends Goal {
        ...
        @Override
        public void tick() {
            ServerLevel level = getServerLevel(lensward);
        ...
        private void tickFiring(ServerLevel level) {
            ...
            if (ticks == 1 && target != null && target.isAlive()) {
                target.hurtServer(
                        level,
                        lensward.damageSources().indirectMagic(lensward, lensward),
                        BEAM_DAMAGE);
            }
