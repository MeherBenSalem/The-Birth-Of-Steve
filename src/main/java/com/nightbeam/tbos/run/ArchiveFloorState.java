...
public record ArchiveFloorState(long floor, List<ArchiveFloorSnapshot> retiredFloors) {
    public static final ArchiveFloorState FIRST = new ArchiveFloorState(0L, List.of());
    ...
    public ArchiveFloorState advance(ArchiveFloorSnapshot completedFloor) {
        ...
        return new ArchiveFloorState(floor + 1L, retired);
    }
    ...
    public ArchiveFloorState removeRetiredSlot(int instanceSlot) {
        List<ArchiveFloorSnapshot> remaining = retiredFloors.stream()
                ...
                .toList();
        return remaining.size() == retiredFloors.size() ? this : new ArchiveFloorState(floor, remaining);
    }
}
