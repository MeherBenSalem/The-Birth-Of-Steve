...
public record ArchiveEncounterState(
        ...
        boolean waveActive,
        int wave,
        int reinforcementBatch,
        int puzzlePhase,
        int puzzleCursor,
        int failures,
        boolean complete) {
    public static final ArchiveEncounterState IDLE =
            new ArchiveEncounterState(false, false, 0, 0, 0, 0, 0, false);
    ...
    public static final Codec<ArchiveEncounterState> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.INT.optionalFieldOf("reinforcement_batch", 0)
                    .forGetter(ArchiveEncounterState::reinforcementBatch),
    ...
    public ArchiveEncounterState {
        ...
        if (reinforcementBatch < 0 || reinforcementBatch > 9) {
            throw new IllegalArgumentException("Archive reinforcement batch must be between 0 and 9");
        }
        ...
        if (!started && (waveActive || wave != 0 || reinforcementBatch != 0
                || puzzlePhase != 0 || puzzleCursor != 0 || failures != 0 || complete)) {
    ...
    /** Source-compatible constructor for pre-v0.3.0 callers and tests. */
    public ArchiveEncounterState(
            boolean started,
            boolean waveActive,
            int wave,
            ...
            int failures,
            boolean complete) {
        this(started, waveActive, wave, 0, puzzlePhase, puzzleCursor, failures, complete);
    }

    public ArchiveEncounterState startWave(int nextWave) {
        return new ArchiveEncounterState(true, true, nextWave, 0, puzzlePhase, puzzleCursor, failures, false);
    }

    public ArchiveEncounterState nextReinforcementBatch() {
        return new ArchiveEncounterState(
                true, true, wave, reinforcementBatch + 1, puzzlePhase, puzzleCursor, failures, false);
    }

    public ArchiveEncounterState startWithoutWave() {
        return new ArchiveEncounterState(
                true, false, wave, reinforcementBatch, puzzlePhase, puzzleCursor, failures, false);
    }
    ...
    public ArchiveEncounterState acceptPuzzleInput() {
        return new ArchiveEncounterState(
                true, waveActive, wave, reinforcementBatch, puzzlePhase, puzzleCursor + 1, failures, false);
    }

    public ArchiveEncounterState rejectPuzzleInput() {
        return new ArchiveEncounterState(
                true, waveActive, wave, reinforcementBatch, puzzlePhase, 0, Math.min(255, failures + 1), false);
    }

    public ArchiveEncounterState finishPuzzleSequence() {
        return new ArchiveEncounterState(true, true, puzzlePhase + 1, 0, puzzlePhase, 0, failures, false);
    }
    ...
    public ArchiveEncounterState finishPuzzleWave() {
        ...
        return nextPhase >= 3
                ? markComplete()
                : new ArchiveEncounterState(true, false, wave, 0, nextPhase, 0, failures, false);
    }

    public ArchiveEncounterState markComplete() {
        return new ArchiveEncounterState(
                true, false, wave, reinforcementBatch, puzzlePhase, puzzleCursor, failures, true);
    }
}
